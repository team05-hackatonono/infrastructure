resource "azurerm_application_insights" "team5" {
...
resource "azurerm_app_service_plan" "team5" {
  name                = var.asp_name
  location            = var.location
  resource_group_name = var.resource_group_name
  sku {
...
resource "azurerm_app_service" "team5" {
  name                = var.wa_name
  location            = var.location
  resource_group_name = var.resource_group_name
  app_service_plan_id = azurerm_app_service_plan.team5.id
